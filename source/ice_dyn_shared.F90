!  SVN:$Id: ice_dyn_shared.F90 700 2013-08-15 19:17:39Z eclare $
!=======================================================================
...
!transport remap "departure point error":
         !if (abs(uvel(i,j) >= vel_max) uvel(i,j) = sign(c1, uvel(i,j))
         !* vel_max
         !if (abs(vvel(i,j) >= vel_max) vvel(i,j) = sign(c1, vvel(i,j))
         !* vel_max
